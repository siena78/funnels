...
$(document).ready(function() {
    var done = false;
    $(".fluffdush_sbutton").on("click", function (e) {
        if (done === true)
        {
            done = false;
            return;
        ...
        e.preventDefault();
        var elem = $(this),
            email = elem.parent().parent().find(".fluffdush_email").val(),
            fname = elem.parent().parent().find(".fluffdush_fname").val(),
            lname = elem.parent().parent().find(".fluffdush_lname").val(),
            phone = elem.parent().parent().find(".fluffdush_phone").val(),
            byeData = {};

        byeData = {
            email: email,
            source: 'centument-de.com'  
        }

        if (typeof fname !== 'undefined') {
            byeData['first_name'] = fname;
        }

        if (typeof lname !== 'undefined') {
            byeData['last_name'] = lname;
        }
        ...
        __bye.collect(byeData, function() {
             done = true;
            elem.trigger('click');
        });
    });
